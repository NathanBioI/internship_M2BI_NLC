# Répartition proportionnelle des espèces de Borrelia selon l'altitude
...
output_table <- "results/repartition_altitude.tsv"
output_plot_png <- "results/repartition_altitude.png"
output_plot_pdf <- "results/repartition_altitude.pdf"
...
altitudes <- c("400m", "800m", "1200m")
colors_altitude <- c("400m" = "#66c2a5", "800m" = "#fc8d62", "1200m" = "#8da0cb")

plot_width <- 22
plot_height <- 9.5
bar_height <- 0.27
dodge_offset <- 0.40
species_spacing <- 1.55
ic_tick_height <- 0.10
...
site_to_altitude <- function(site) {
  site <- as.character(site)
  case_when(
    str_detect(site, regex("S12|400", ignore_case = TRUE)) ~ "400m",
    str_detect(site, regex("S13|800", ignore_case = TRUE)) ~ "800m",
    str_detect(site, regex("S14|1200", ignore_case = TRUE)) ~ "1200m",
    TRUE ~ NA_character_
  )
}
...
plot_data <- counts %>%
  ...
  mutate(
    altitude = factor(altitude, levels = altitudes),
    y_pos = case_when(
      altitude == "400m" ~ y_base + dodge_offset,
      altitude == "800m" ~ y_base,
      altitude == "1200m" ~ y_base - dodge_offset,
      TRUE ~ y_base
    ),
...
y_min <- min(df_y$y_base) - 0.85
y_max <- max(df_y$y_base) + 0.85
...
p <- ggplot() +
  geom_rect(
    data = plot_data,
    aes(xmin = xmin, xmax = xmax, ymin = ymin, ymax = ymax, fill = altitude),
  ...
  geom_richtext(
    ...
    hjust = 0, size = 4.2, color = "black", fill = NA, label.color = NA
  ) +
  geom_richtext(
    data = labels_species,
  ...
  scale_fill_manual(values = colors_altitude, drop = FALSE) +
  ...
  labs(
    title = "Répartition proportionnelle des espèces selon l'altitude",
